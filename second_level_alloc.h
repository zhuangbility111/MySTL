...
enum {__ALIGN = 8};     // 小型内存块的上调边界，也就是每一个内存块必须是__ALIGN的倍数
enum {__MAX_BYTES = 128};       // 小型内存块的上限，也就是内存块的最大尺寸
enum {__NFREELISTS = __MAX_BYTES / __ALIGN};        // free-lists的个数，也就是总共有多少种不同大小的内存块，每一种内存块都有一个链表free-list负责管理
